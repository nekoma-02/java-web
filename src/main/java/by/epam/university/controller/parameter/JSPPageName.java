package by.epam.university.controller.parameter;

public class JSPPageName {
	public static final String USER_PAGE = "jsp/userPage.jsp";
	public static final String UPDATE_FACULTY = "jsp/admin/update_faculty.jsp";
	public static final String UPDATE_SCHOOL = "jsp/admin/update_school.jsp";
	public static final String UPDATE_SPECIALTY = "jsp/admin/update_specialty.jsp";
	public static final String UPDATE_PRIVILEGE = "jsp/admin/update_privilege.jsp";
	public static final String UPDATE_TYPE_STUDY = "jsp/admin/update_typeStudy.jsp";
	public static final String ERROR_PAGE = "error.jsp";
	public static final String INDEX_PAGE = "index.jsp";
	public static final String REGISTRATION_PAGE = "jsp/registration.jsp";
	public static final String LOGIN_PAGE = "jsp/login.jsp";
	public static final String INFO_PAGE = "jsp/specialty.jsp";
	public static final String SPECIALTY_DETAIL = "jsp/specialty_detail.jsp";
	public static final String ADD_APPLICATION_PAGE = "jsp/add_application.jsp";
	public static final String ADMIN_PAGE = "WEB-INF/jsp/admin_page.jsp";
	
	public static final String ADD_FACULTY = "jsp/admin/add_faculty.jsp";
	public static final String ADD_SCHOOL = "jsp/admin/add_school.jsp";
	public static final String ADD_SPECIALTY = "jsp/admin/add_specialty.jsp";
	public static final String ADD_PRIVILEGE = "jsp/admin/add_privilege.jsp";
	public static final String ADD_TYPE_STUDY = "jsp/admin/add_typeStudy.jsp";
}
